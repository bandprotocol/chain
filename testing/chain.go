...
func NewTestChain(t *testing.T, chainID string) *TestChain {
	...
	err = chain.App.ScopedOracleKeeper.ClaimCapability(chain.GetContext(), cap, host.PortPath(MockPort))
	require.NoError(t, err)
...
func (chain *TestChain) NextTestChannel(conn *ibctesting.TestConnection, portID string) ibctesting.TestChannel {
...
func (chain *TestChain) CreatePortCapability(portID string) {
	...
	if !ok {
		...
		// 	require.NoError(chain.t, err)
		case OraclePort:
			err = chain.App.ScopedOracleKeeper.ClaimCapability(chain.GetContext(), cap, host.PortPath(portID))
			require.NoError(chain.t, err)
		default:
...
func (chain *TestChain) CreateChannelCapability(portID, channelID string) {
	...
	if !ok {
		...
		require.NoError(chain.t, err)
		err = chain.App.ScopedOracleKeeper.ClaimCapability(chain.GetContext(), cap, capName)
		require.NoError(chain.t, err)
	}
