...
func CreateTestApp(t *testing.T, autoActivate bool) (*TestingApp, sdk.Context) {
	...
	if autoActivate {
		...
		// active tss status
		err = app.TSSKeeper.HandleSetDEs(ctx, Validators[0].Address, []tsstypes.DE{
			{
		...
		require.NoError(t, err)
		err = app.TSSKeeper.HandleSetDEs(ctx, Validators[1].Address, []tsstypes.DE{
			{
				PubD: testutil.HexDecode("dddd"),
				PubE: testutil.HexDecode("eeee"),
			},
		...
		require.NoError(t, err)
		err = app.TSSKeeper.HandleSetDEs(ctx, Validators[2].Address, []tsstypes.DE{
			{
				PubD: testutil.HexDecode("dddd"),
				PubE: testutil.HexDecode("eeee"),
			},
...
>>>>>>> ef9fa5a03d1290601c9ef58de55c460805a2fdc7:testing/test_helpers.go
	if withGenesis {
