...
func (suite *DecoderTestSuite) TestDecodeBandtssMsgUpdateParams() {
	...
	msg := bandtsstypes.MsgUpdateParams{
		Params: bandtsstypes.NewParams(100, 10, 50, 50, sdk.Coins{Amount}),
	}

	emitter.DecodeBandtssMsgUpdateParams(&msg, detail)
	expectedJSON := `{"active_duration":100,"authority":"","fee":[{"denom":"uband","amount":"1"}],"inactive_penalty_duration":50,"max_transition_duration":50,"reward_percentage":10}`
	suite.testCompareJson(detail, expectedJSON)
}
