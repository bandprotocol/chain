...
func (h *Hook) getCurrentRewardAndCurrentRatio(ctx sdk.Context, addr sdk.ValAddress) (string, string) {
	...
	reward, _ := h.distrKeeper.GetValidatorCurrentRewards(ctx, addr)
	latestReward, _ := h.distrKeeper.GetValidatorHistoricalRewards(ctx, addr, reward.Period-1)
...
func (h *Hook) emitUpdateValidatorRewardAndAccumulatedCommission(ctx sdk.Context, addr sdk.ValAddress) {
	...
	valCommission, _ := h.distrKeeper.GetValidatorAccumulatedCommission(ctx, addr)
	accCommission, _ := valCommission.Commission.TruncateDecimal()
...
func (h *Hook) handleMsgWithdrawDelegatorReward(
	ctx sdk.Context, msg *types.MsgWithdrawDelegatorReward, evMap common.EvMap, detail common.JsDict,
) {
	valAddr, _ := sdk.ValAddressFromBech32(msg.ValidatorAddress)
	delAddr, _ := sdk.AccAddressFromBech32(msg.DelegatorAddress)
	withdrawAddr, _ := h.distrKeeper.GetDelegatorWithdrawAddr(ctx, delAddr)
	...
	detail["reward_amount"] = evMap[types.EventTypeWithdrawRewards+"."+sdk.AttributeKeyAmount][0]
}

// handleMsgSetWithdrawAddress implements emitter handler for MsgSetWithdrawAddress.
func (h *Hook) handleMsgSetWithdrawAddress(msg *types.MsgSetWithdrawAddress, detail common.JsDict) {
...
func (h *Hook) handleMsgWithdrawValidatorCommission(
	ctx sdk.Context, msg *types.MsgWithdrawValidatorCommission, evMap common.EvMap, detail common.JsDict,
) {
	...
	detail["moniker"] = val.Description.Moniker
	detail["identity"] = val.Description.Identity
	detail["commission_amount"] = evMap[types.EventTypeWithdrawCommission+"."+sdk.AttributeKeyAmount][0]
}
