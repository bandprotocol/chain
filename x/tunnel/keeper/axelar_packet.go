...
func (k Keeper) GetAxelarPacket(ctx sdk.Context, tunnelID uint64, packetID uint64) (types.AxelarPacket, error) {
	bz := ctx.KVStore(k.storeKey).Get(types.TunnelPacketStoreKey(tunnelID, packetID))
	if bz == nil {
		return types.AxelarPacket{}, types.ErrPacketNotFound.Wrapf("tunnelID: %d, packetID: %d", tunnelID, packetID)
	}
