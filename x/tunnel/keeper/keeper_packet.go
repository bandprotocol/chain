...
func (k Keeper) ProducePacket(
	...
	newSignalPrices, err := GenerateNewSignalPrices(
		latestSignalPrices,
...
func (k Keeper) SendPacket(ctx sdk.Context, packet types.Packet) error {
	...
	case *types.IBCRoute:
		content, fee, err = k.SendIBCPacket(ctx, r, packet)
	case *types.RouterRoute:
		content, fee, err = k.SendRouterPacket(ctx, r, packet, tunnel.Encoder, sdk.MustAccAddressFromBech32(tunnel.FeePayer))
	default:
