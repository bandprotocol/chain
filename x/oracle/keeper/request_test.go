...
func TestProcessExpiredRequests(t *testing.T) {
	...
	require.True(t, k.GetValidatorStatus(ctx, bandtesting.Validators[0].ValAddress).IsActive)
	require.True(t, k.GetValidatorStatus(ctx, bandtesting.Validators[1].ValAddress).IsActive)
	...
	require.Equal(t, sdk.Events{sdk.NewEvent(
		...
		sdk.NewAttribute(types.AttributeKeyValidator, bandtesting.Validators[1].ValAddress.String()),
