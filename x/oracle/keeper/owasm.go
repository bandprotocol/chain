...
func (k Keeper) GetRandomValidators(ctx sdk.Context, size int, id int64) ([]sdk.ValAddress, error) {
	...
	tryCount := int(k.GetUInt64Param(ctx, types.KeySamplingTryCount))
...
func (k Keeper) PrepareRequest(
	...
	if askCount > k.GetUInt64Param(ctx, types.KeyMaxAskCount) {
		return 0, sdkerrors.Wrapf(types.ErrInvalidAskCount, "got: %d, max: %d", askCount, k.GetUInt64Param(ctx, types.KeyMaxAskCount))
	}

	// Consume gas for data requests.
	ctx.GasMeter().ConsumeGas(askCount*k.GetUInt64Param(ctx, types.KeyPerValidatorRequestGas), "PER_VALIDATOR_REQUEST_FEE")
	...
	env := types.NewPrepareEnv(req, int64(k.GetUInt64Param(ctx, types.KeyMaxRawRequestCount)))
	...
	ctx.GasMeter().ConsumeGas(k.GetUInt64Param(ctx, types.KeyBaseOwasmGas), "BASE_OWASM_FEE")
