...
func TestHasDataSource(t *testing.T) {
	...
	k.SetDataSource(ctx, 42, types.NewDataSource(
		testapp.Owner.Address, BasicName, BasicDesc, BasicFilename, testapp.Treasury.Address, testapp.EmptyCoins,
	))
...
func TestSetterGetterDataSource(t *testing.T) {
	...
	dataSource1 := types.NewDataSource(testapp.Alice.Address, "NAME1", "DESCRIPTION1", "filename1", testapp.Treasury.Address, testapp.EmptyCoins)
	dataSource2 := types.NewDataSource(testapp.Bob.Address, "NAME2", "DESCRIPTION2", "filename2", testapp.Treasury.Address, testapp.EmptyCoins)
...
func TestAddDataSourceEditDataSourceBasic(t *testing.T) {
	...
	k.MustEditDataSource(ctx, id, types.NewDataSource(
		owner, dataSource2.Name, dataSource2.Description, dataSource2.Filename, testapp.Treasury.Address, testapp.EmptyCoins,
	))
...
func TestEditDataSourceDoNotModify(t *testing.T) {
	...
	// Creates some basic data sources.
	dataSource1 := types.NewDataSource(testapp.Alice.Address, "NAME1", "DESCRIPTION1", "FILENAME1", testapp.Treasury.Address, testapp.EmptyCoins)
	dataSource2 := types.NewDataSource(testapp.Bob.Address, types.DoNotModify, types.DoNotModify, "FILENAME2", testapp.Treasury.Address, testapp.EmptyCoins)
	// Adds a new data source to the store. We should be able to retreive it back.
	id := k.AddDataSource(ctx, dataSource1)
	require.Equal(t, dataSource1, k.MustGetDataSource(ctx, id))
	require.NotEqual(t, dataSource2, k.MustGetDataSource(ctx, id))
...
func TestAddDataSourceDataSourceMustReturnCorrectID(t *testing.T) {
	...
	id1 := k.AddDataSource(ctx, types.NewDataSource(testapp.Owner.Address, BasicName, BasicDesc, BasicFilename, testapp.Treasury.Address, testapp.EmptyCoins))
	require.Equal(t, types.DataSourceID(genesisCount+1), id1)
	// Adds another data source so now ID should increase by 2.
	id2 := k.AddDataSource(ctx, types.NewDataSource(testapp.Owner.Address, BasicName, BasicDesc, BasicFilename, testapp.Treasury.Address, testapp.EmptyCoins))
