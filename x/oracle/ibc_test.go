...
func (suite *OracleTestSuite) TestHandleIBCRequestSuccess() {
	...
	suite.chainB.SendReport(1, raws1, bandtesting.Validators[0])
	suite.Require().NoError(err)
	...
	suite.chainB.SendReport(1, raws2, bandtesting.Validators[1])
	suite.Require().NoError(err)
...
func (suite *OracleTestSuite) TestIBCResolveRequestOutOfGas() {
	...
	suite.chainB.SendReport(1, raws, bandtesting.Validators[0])
