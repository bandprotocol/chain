...
func TestGetBytesRequestPacket(t *testing.T) {
	req := OracleRequestPacketData{
		...
		AskCount:       1,
		MinCount:       1,
		FeeLimit:       sdk.NewCoins(sdk.NewCoin("odin", sdk.NewInt(10000))),
		RequestKey:     "TEST_KEY",
		PrepareGas:     100,
		ExecuteGas:     100,
	}
	require.Equal(t,
		[]byte(`{"ask_count":"1","calldata":"AwAAAEJUQ2QAAAAAAAAA","client_id":"test","execute_gas":"100","fee_limit":[{"amount":"10000","denom":"odin"}],"min_count":"1","oracle_script_id":"1","prepare_gas":"100","request_key":"TEST_KEY"}`),
		req.GetBytes(),
	)
}
