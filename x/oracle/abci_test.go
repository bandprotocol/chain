...
func TestAllocateTokensCalledOnBeginBlock(t *testing.T) {
	...
	app.BankKeeper.SetBalance(ctx, app.AccountKeeper.GetModuleAddress(authtypes.FeeCollectorName), sdk.NewInt64Coin("odin", 100))
	...
	require.Equal(t, sdk.NewCoins(sdk.NewInt64Coin("odin", 100)), app.BankKeeper.GetAllBalances(ctx, app.AccountKeeper.GetModuleAddress(authtypes.FeeCollectorName)))
	...
	require.Equal(t, sdk.NewCoins(sdk.NewInt64Coin("odin", 30)), app.BankKeeper.GetAllBalances(ctx, app.AccountKeeper.GetModuleAddress(authtypes.FeeCollectorName)))
	require.Equal(t, sdk.NewCoins(sdk.NewInt64Coin("odin", 70)), app.BankKeeper.GetAllBalances(ctx, app.AccountKeeper.GetModuleAddress(distrtypes.ModuleName)))
	// 100*70%*2% = 1.4odin
	...
	// 100*70%*98% = 68.6odin
	require.Equal(t, distrtypes.ValidatorOutstandingRewards{Rewards: sdk.NewDecCoins(sdk.NewDecCoinFromDec("odin", sdk.NewDecWithPrec(686, 1)))}, app.DistrKeeper.GetValidatorOutstandingRewards(ctx, testapp.Validators[1].ValAddress))
	...
	require.Equal(t, sdk.NewCoins(sdk.NewInt64Coin("odin", 9)), app.BankKeeper.GetAllBalances(ctx, app.AccountKeeper.GetModuleAddress(authtypes.FeeCollectorName)))
	require.Equal(t, sdk.NewCoins(sdk.NewInt64Coin("odin", 91)), app.BankKeeper.GetAllBalances(ctx, app.AccountKeeper.GetModuleAddress(distrtypes.ModuleName)))
	...
	require.Equal(t, distrtypes.ValidatorOutstandingRewards{Rewards: sdk.NewDecCoins(sdk.NewDecCoinFromDec("odin", sdk.NewDecWithPrec(14406, 3)))}, app.DistrKeeper.GetValidatorOutstandingRewards(ctx, testapp.Validators[0].ValAddress))
	// 68.6odin + 30*70%*98%*30% = 74.774odin
	require.Equal(t, distrtypes.ValidatorOutstandingRewards{Rewards: sdk.NewDecCoins(sdk.NewDecCoinFromDec("odin", sdk.NewDecWithPrec(74774, 3)))}, app.DistrKeeper.GetValidatorOutstandingRewards(ctx, testapp.Validators[1].ValAddress))
	...
	require.Equal(t, sdk.NewCoins(sdk.NewInt64Coin("odin", 3)), app.BankKeeper.GetAllBalances(ctx, app.AccountKeeper.GetModuleAddress(authtypes.FeeCollectorName)))
	require.Equal(t, sdk.NewCoins(sdk.NewInt64Coin("odin", 97)), app.BankKeeper.GetAllBalances(ctx, app.AccountKeeper.GetModuleAddress(distrtypes.ModuleName)))
	...
	require.Equal(t, distrtypes.ValidatorOutstandingRewards{Rewards: sdk.NewDecCoins(sdk.NewDecCoinFromDec("odin", sdk.NewDecWithPrec(20286, 3)))}, app.DistrKeeper.GetValidatorOutstandingRewards(ctx, testapp.Validators[0].ValAddress))
	// 74.774odin
	require.Equal(t, distrtypes.ValidatorOutstandingRewards{Rewards: sdk.NewDecCoins(sdk.NewDecCoinFromDec("odin", sdk.NewDecWithPrec(74774, 3)))}, app.DistrKeeper.GetValidatorOutstandingRewards(ctx, testapp.Validators[1].ValAddress))
}
...
func TestAllocateTokensWithDistrAllocateTokens(t *testing.T) {
	...
	// Set collected fee to 100odin + 70% oracle reward proportion + disable minting inflation.
	feeCollector := app.AccountKeeper.GetModuleAccount(ctx, authtypes.FeeCollectorName)
	app.BankKeeper.SetBalance(ctx, app.AccountKeeper.GetModuleAddress(authtypes.FeeCollectorName), sdk.NewInt64Coin("odin", 50))
	app.AccountKeeper.SetAccount(ctx, feeCollector)
	mintParams := app.MintKeeper.GetParams(ctx)
	mintParams.InflationMin = sdk.ZeroDec()
	mintParams.InflationMax = sdk.ZeroDec()
	...
	require.Equal(t, sdk.NewCoins(sdk.NewInt64Coin("odin", 50)), app.BankKeeper.GetAllBalances(ctx, app.AccountKeeper.GetModuleAddress(authtypes.FeeCollectorName)))
	...
	require.Equal(t, sdk.NewCoins(sdk.NewInt64Coin("odin", 50)), app.BankKeeper.GetAllBalances(ctx, app.AccountKeeper.GetModuleAddress(distrtypes.ModuleName)))
	require.Equal(t, sdk.DecCoins{{Denom: "odin", Amount: sdk.NewDec(1)}}, app.DistrKeeper.GetFeePool(ctx).CommunityPool)
	require.Equal(t, distrtypes.ValidatorOutstandingRewards{Rewards: sdk.NewDecCoins(sdk.NewDecCoinFromDec("odin", sdk.NewDecWithPrec(43015, 3)))}, app.DistrKeeper.GetValidatorOutstandingRewards(ctx, testapp.Validators[0].ValAddress))
	require.Equal(t, distrtypes.ValidatorOutstandingRewards{Rewards: sdk.NewDecCoins(sdk.NewDecCoinFromDec("odin", sdk.NewDecWithPrec(5985, 3)))}, app.DistrKeeper.GetValidatorOutstandingRewards(ctx, testapp.Validators[1].ValAddress))
}
