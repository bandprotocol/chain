...
// 		err = Verify(&gotSig.R, &gotSig.S, &commitment, pubKey, nil)
// 		if err != nil {
...
// 		var sigR secp256k1.JacobianPoint
// 		secp256k1.ScalarBaseMultNonConst(nonce, &sigR)
// 		sigS, err := ComputeSigS(&privKey.Key, nonce, &commitment)
// 		if err != nil {
...
// 		if err := Verify(&sig.R, &sig.S, &commitment, pubKey, nil); err != nil {
...
// 		var badCommitment secp256k1.ModNScalar
// 		badCommitment.SetByteSlice(badHash)
// 		if err := Verify(&sig.R, &sig.S, &badCommitment, pubKey, nil); err == nil {
...
// 		var commitment secp256k1.ModNScalar
// 		commitment.SetByteSlice(hash)
// 		err = Verify(&sig.R, &sig.S, &commitment, pubKey, nil)
// 		if !errors.Is(err, test.err) {
// 			t.Errorf("%s: mismatched err -- got %v, want %v", test.name, err,
// 				test.err)
// 			continue
